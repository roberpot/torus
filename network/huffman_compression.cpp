...
const word_t HuffmanCompression::compression_table[COMPRESS_TREE_SIZE] =	// static
{
...
udword_t HuffmanCompression::compress(t_byte* pOutput, const t_byte* pInput, udword_t expected_len, udword_t input_len)
{
    ADDTOCALLSTACK();

    uint output_len = 0;
    int bitidx = 0;	    // Offset in output byte (output_value)
    t_byte output_value = 0;

    for (uint i = 0; i <= input_len; ++i)
    {
        word_t value = compression_table[ ( i == input_len ) ? (COMPRESS_TREE_SIZE - 1) : pInput[i] ];
        int nBits = value & 0xF;
        value >>= 4;
        while ( nBits-- )
        {
            ...
            output_value <<= 1;
            output_value |= (value >> nBits) & 0x1;
            if ( ++bitidx == 8)
            {
                bitidx = 0;
                pOutput[output_len++] = output_value;
            }
        }
    }
    if (bitidx)	// flush odd bits.
    {
        if (output_len >= expected_len)
            return 0;
        pOutput[output_len++] = output_value << (8 - bitidx);
    }
