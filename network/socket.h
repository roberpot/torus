...
class Socket {
 ...
 public:
  Socket(socket_t s,
         ConnectionType connection_type = ConnectionType::CONNECTIONTYPE_CLIENT,
...
*       byte[30]	Password
*       *Notes: Acc and password were already receipt, but this is a double check since the login server and game server
maybe different, so both should do the check. Auth ID is an additional security token providen for the loginserver and
should be expected by the gameserver.
...
*   int32_t flags (for character creation)*:
    ...
    *Each flag is for each feature, if you need to combine features, you need to summ flags.
        ...
        0x8000 flag is used for unlocking new Felucca factions areas, note that you have to use "_x" versions of
map/statics if you want to move through new areas.
    **Last character slot for SA 3D clients: it will highlight last character used.

*    Since 7.0.13.0 and 4.0.13.0 City Name and Building Name have length of 32 chars, also added city x,y,z,map,cliloc
description and dword 0 to city structure.
*/
