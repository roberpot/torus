...
PacketIn::PacketIn(bool has_dynamic_length) : _is_complete(false), _has_dynamic_length(has_dynamic_length) {
}
...
const uword_t PacketIn::receive(const uint8_t* data, const uword_t len) {
  ...
  if (_current_buffer_length >= expected_length) {
    ...
    if (_has_dynamic_length) {
      ...
      _current_pos =
          1;  // Move the cursor, so the id is not read when processing it's data in the packets' specific code.
    }
  }
  TORUSSHELLECHO("Packet " << this << " receive data (0x" << hex(_buffer[0]) << ")[" << std::dec
                           << _current_buffer_length << "] = " << std::endl
                           << hex_dump_buffer(_buffer, _current_buffer_length));
  return readed_bytes;
}
...
const std::string PacketIn::read_string(uword_t len) {
  ...
  if (_current_pos + len > length()) {
    // THROW_ERROR(NetworkError, "Trying to read " << sizeof(T) << " bytes to from " << hex(p._buffer[0]) << ", being
    // currently in the position " << p._current_pos << " into a string with a total size of " << s.size() " bytes.");
    TORUSSHELLECHO("Trying to read " << len << " bytes to from " << hex(_buffer[0])
                                     << ", being currently in the position " << _current_pos
                                     << " into a string with a total size of " << _current_buffer_length << " bytes.");
    return str;
  }
  str.resize(len);
  int i = 0;
...
const word_t PacketIn::read_word() {
  word_t ret = word_t(_buffer[_current_pos] << 8 | _buffer[_current_pos + 1]);
  _current_pos += 2;
  return ret;
}

const uword_t PacketIn::read_uword() {
  uword_t ret = uword_t(_buffer[_current_pos] << 8 | _buffer[_current_pos + 1]);
  _current_pos += 2;
  return ret;
}

const wchar_t PacketIn::read_wchar() {
  wchar_t ret = ((_buffer[_current_pos] << 8) | _buffer[_current_pos + 1]);
  _current_pos += 2;
  return ret;
}

const dword_t PacketIn::read_dword() {
  dword_t ret = dword_t((_buffer[_current_pos] << 24) | (_buffer[_current_pos + 1] << 16) |
                        (_buffer[_current_pos + 2] << 8) | _buffer[_current_pos + 3]);
  _current_pos += 4;
  return ret;
}

const udword_t PacketIn::read_udword() {
  udword_t ret = udword_t((_buffer[_current_pos] << 24) | (_buffer[_current_pos + 1] << 16) |
                          (_buffer[_current_pos + 2] << 8) | _buffer[_current_pos + 3]);
  _current_pos += 4;
  return ret;
}
