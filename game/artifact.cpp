...
Artifact::Artifact(UidMask mask) : Artifact() {
  _uid.find_new_uid();
  _uid.set_uid_type(mask);
}
...
void Artifact::move_to(word_t destX, word_t destY) {
  ...
  if (!_position.can_move_to_coord(destX, destY)) {
    // DEBUG_ERROR("Trying to move 0x" << std::hex << get_uid().get_uid() << "to a non-valid dest " << std::dec << destX
    // << ", " << destY); DEBUG_ERROR("Trying to move to a non-valid dest " << std::dec << destX << ", " << destY);
...
void Artifact::set_z(t_byte destZ) {
  ...
  if (!_position.can_move_to_z(destZ)) {
    DEBUG_ERROR("Trying to move 0x" << std::hex << get_uid().get_uid() << " out of limits (" << destZ
                                    << "), avoiding it.");
    return;
  }
